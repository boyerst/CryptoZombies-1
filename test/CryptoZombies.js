...
// Import chai before use assertion library
...
contract("CryptoZombies", (accounts) => {
    ...
    it("zombies should be able to attack another zombie", async () => {
      let result;
      // Declare result which contains Logs and Events + tx hash, tx receipt, and more
        // 1. Create zombie for alice
      result = await contractInstance.createRandomZombie(zombieNames[0], {from: alice});
        // Grab the zombieId from the logs generated by Ganache
      const firstZombieId = result.logs[0].args.zombieId.toNumber();
        // 2. Create zombie for bob
      result = await contractInstance.createRandomZombie(zombieNames[1], {from: bob});
        // Grab the zombieId from the logs generated by Ganache
      const secondZombieId = result.logs[0].args.zombieId.toNumber();
      // We call our helper function named 'days' to increase time in order to bypas cooldown period
      await time.increase(time.duration.days(1))
      // Alice attacks her zombie with Bobs zombie as a parameter
      await contractInstance.attack(firstZombieId, secondZombieId, {from: alice});
      // For the test to pass we check if reslut.receipt.status = true
      expect(result.receipt.status).to.equal(true);
    })
